    ...
    $.ajax({
      ...
      success:function(v) {
        ...
        BootstrapDialog.show({
          title : 'LINK ',
          message : $('<div class="container-fluid"><p>Comparte el link : <a href="' + bit_url + '" target = "_blank">' + bit_url + '</a><p>' + 
            '<div class="col-lg-12" style="text-align : center; padding-bottom : 90px;">' + 
            '<share-button class="share-button" style="position : absolute; top: 0px; left : 0; right : 0;"></share-button>' +
          '</div></div>'),
          onshow : function(dialog){
            dialog.getModalHeader().replaceWith($('<div class="row" style="margin: 0px; padding-top: 5px; border-top-left-radius: 10px; border-top-right-radius: 10px; background: url(&#39;http://www.batlleiroig.com/wp-content/uploads/247_parc_central_st_cugat_8.jpg&#39;); background-size: cover; background-repeat: no-repeat;">' + 
              ...
              '</div>' +
              '<div class="col-xs-12" style="text-align: center; color: #fff; font-weight : bold;">' +
