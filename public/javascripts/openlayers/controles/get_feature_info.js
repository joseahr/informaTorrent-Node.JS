...
app.GetFeatureInfo = function(opt_options) {
  ...
  addURL = function(coordinate, layer){
    if(layer.getSource() instanceof ol.source.TileWMS && layer.getVisible() == true){
      ...
      .done(function(data){
        console.log(data);
        if(data.indexOf('table') == -1) return;
        $(div).append(data);
      })
  ...
  function getListenerInfo(){
    return map.on('singleclick', function(evt){ // Cuando clickemos en el mapa 
      ...
      div.append('<div class="col-lg-12 text-center"><p style="background : #00bbff; color : #fff; border-radius : 5px;">' + ol.coordinate.toStringHDMS(coordinate) + '</p></div>');
